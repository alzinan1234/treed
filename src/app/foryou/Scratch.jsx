...
const Scratch = () => {
    ...
    return (
        <div className='w-10/12 mx-auto mb-42'>
            <div className='flex flex-col items-center p-8'>
                <h1 className="mb-5 text-xl md:text-6xl font-bold text-center w-full md:w-3/4 mx-auto">
                    Built 100% from Scratch
                </h1>
                    <p className="mb-5 w-full md:w-1/2 text-center text-sm md:text-base text-[#B0B0B0]">
                    TreediMap will be created without using a single external API. Every road, every building, every detail will be mapped independently to deliver the most complete, accurate, and accessible map ever made in Canada — for millions of Canadians and visitors, to simplify daily life like never before.
                </p>
            </div>

            {/* Desktop View (Unchanged) */}
            <div className="hidden md:block lg:flex lg:items-start lg:flex-col">
                <div className="relative z-10 overflow-hidden">
                    <div className="w-full flex justify-center gap-4 z-20 ml-42">
                        {features.map((feature, index) => (
                            <div
                                key={index}
                                className={`bg-[#161616] border border-[#404040] rounded-2xl p-4 text-white transition-all duration-300 ${current === index ? 'ring-2 ring-[#9155F7]' : ''}`}
                            >
                                {feature.icon}
                ...
                <div className=" flex justify-center ml-42 mt-10 gap-6 z-30">
                    <button
                        onClick={handlePrev}
            ...
            <div className="md:hidden flex flex-col items-start">
                <div className="bg-[#0D0D0D] border border-[#404040] rounded-2xl p-6 text-white">
                    {features[current].icon}
                    <h3 className="font-semibold text-lg mb-2">{features[current].title}</h3>
                    <p className="text-[#B0B0B0] text-sm">{features[current].description}</p>
                </div>
